...
export default function Overview() {
  ...
  const config = (key) => {
    const a = analysis();
    if (a) if (a.config) return a.config[key];
  }

  const [searchParams, setSearchParams] = useSearchParams();
  const initViewMode = searchParams.view ? searchParams.view : "graph";
  ...
  const initTimeLater= searchParams.later ? parseInt(searchParams.later) : 3600000;
  ...
  const [set] = createResource(analysis, async (input) => {
    return await read_set(resourceServer.get(apiId()), { id: input.set_id })
  });
  const [model_config] = createResource(set, async (input) => {
    const model_ids = input.members.map(member => member.model_id);
    const models = await list_model_by_ids(resourceServer.get(apiId()), { ids: model_ids });
    let configs = [];
  ...
  const [device_config] = createResource(set, async (input) => {
    ...
    const devices = await list_device_by_ids(resourceServer.get(apiId()), { ids: device_ids });
    let configs = [];
    for (const member of input.members) {
  ...
  const [dataset, {refetch}] = createResource(analysis, async (input) => {
    if (timeMode() == "live") {
      const tLater = new Date(Date.now() - timeLater());
      return await list_data_set_by_range(resourceServer.get(apiId()), {
        set_id: input.set_id,
        begin: tLater,
      ...
      return await read_data_set(resourceServer.get(apiId()), {
        set_id: input.set_id,
      ...
      <div class="w-full max-w-[48rem] xs:rounded-sm border border-slate-200 dark:border-slate-700">
        <div class="w-full flex flex-row items-center justify-between bg-gray-100 dark:bg-gray-800">
          <div class="mx-2 my-1.5 flex flex-row items-center font-semibold">
            <span class={(analysis() ? analysis().icon : "icon-list_square") + " text-[1.5rem] align-middle"}></span>
            <span class="ml-1 align-middle">{analysis() ? analysis().name : ""}&nbsp;</span>
          </div>
            ...
            <div class="xs:rounded-sm border border-slate-200 dark:border-slate-700">
              <div class="flex flex-row items-center bg-gray-100 dark:bg-gray-800">
                <div class="mx-3 my-1.5 flex flex-row items-center font-medium">
                  <span class="align-middle text-sm">{analysis().name}&nbsp;</span>
    ...
    <Show when={viewMode() == "table"}>
      <div class="w-full xs:px-1 py-1 overflow-hidden">
        <div class="w-full max-w-[48rem] xs:rounded-sm border border-slate-200 dark:border-slate-700">
          <div class="flex flex-row items-center bg-gray-100 dark:bg-gray-800">
            <div class="mx-3 my-1.5 flex flex-row items-center font-medium">
              <span class="align-middle text-sm leading-6">{analysis() ? analysis().name : ""}&nbsp;</span>
            </div>
          </div>
