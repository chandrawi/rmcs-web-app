...
export default function SparingSensors(props) {
  ...
  const initFrameMode = searchParams.frame ? searchParams.frame : "minutely";
  ...
  /**
   * @returns {{ timestamp: Date|undefined, data: number[], group: string }[]}
   */
  const datasetMap = () => {
      ...
      for (const dataset of datasets) {
        ...
        for (const member of sets.members) {
          if (dataset.data.length >= (index + member.data_index.length)) {
            dataMap.data.push(dataset.data.slice(index, index + member.data_index.length));
          }
